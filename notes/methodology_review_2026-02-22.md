# Methodology Review Discussion – Dynamic Risk Management: Cyber-Physical Research Pipeline
...
- Focus: Dynamic Risk Management in critical infrastructure, centered on the “Break-Glass” intersection where cybersecurity lockdowns may be degraded to protect life or prevent physical disasters.
...
## 2. README – Overall Pipeline and Methodology
...
- Core focus: the **“Break-Glass” intersection** – emergency moments where cyber controls may need to be intentionally degraded/bypassed to:
  - Preserve “All-Souls-on-Site” (life safety)  
...
### 2.2 Three-Tier “Dynamic Switch” Logic

The pipeline evaluates global frameworks (NIST, ISO, FEMA, etc.) through a three-tier logic:

- **Tier 1 – Manned Facilities (High Proximity)**  
  - Context: Hospitals, transport hubs, other manned facilities.  
  - Rule: **Human/Soul-First** – security must fail-open when necessary so it does not impede escape routes or life-support for “All-Souls-on-Site”.

- **Tier 2 – Remote Sites with Downstream Risk**  
  - Context: Remote power substations, dams, offshore ROVs, etc.  
  - Rule: **Balanced** – prioritize asset security specifically to prevent secondary community-level disasters or environmental hazards.

- **Tier 3 – Isolated/Unmanned Sites**  
  - Context: Isolated environments with no direct or downstream human/physical risk.  
  - Rule: **Asset-First** – prioritize strict mission continuity and hardware integrity.
...
1. **Step 1: Discovery (`scout.py`)**
   - A pluggable hybrid AI agent that:
     ...
     - Logs low-quality hits in a “smart memory” file (`seen_sources.txt`) for auditing and avoiding duplicates.
   ...
   - **Reproducible memory**:
     - The “seen_sources.txt” log is tracked in Git as an auditable history of:
       - Queries executed
...
## 3. PROJECT_RULES – Master Rules and Logic
...
- Tier 1 (High Proximity, Direct Life-Safety) → Human/Soul-First.
- Tier 2 (Remote/Unmanned with Downstream Risk) → Balanced; asset security used to prevent second-/third-order physical harms.
- Tier 3 (Isolated/Unmanned) → Asset-First.

### 3.3 Evolution Directive (Master Clause)

- The framework is **explicitly iterative**.
- The AI must use the 🚩`[PROMPT_EVOLUTION_TRIGGER]` flag whenever it encounters:
...
- 🚩 `[REGULATORY_BARRIER]`:  
  Legal/compliance barriers to “flexible” overrides.

- 🔍 `[SPOOF_VULNERABILITY]`:  
  Sensor vulnerabilities that could be exploited to force a fail-open state.

- 🔄 `[INCIDENT_FEEDBACK_LOOP]`:  
  Requirements or mechanisms for post-incident learning/evolution (system “DNA” updates after break-glass events).

### 3.6 Logic Dictionary (Technical Translation Layer)

A mapping between thesis terms and global frameworks’ vocabulary:
...
## 4. auditor.md – Skill: Senior Cyber-Physical Resilience Auditor
...
- **Grey Literature Override**:
  - Process blogs/expert opinions only if they discuss “Dynamic Risk” architecture, logic, or framework gaps.
  - If they pass but lack empirical data/architecture diagrams → flag 🚩`[THEORETICAL_ONLY]`.
- Exit Strategy:
  - If neither dynamic logic nor gaps are present: 3-sentence summary and stop.

### 4.2 Step 1: Ingestion & Language Gate

- Only process English or English-translated sources → else `[SKIP_NON_ENGLISH]`.
- IT-Centric check:
...
- 70% parity rule:
  - A mapping is valid if **3 of 4** criteria match:
    ...
    - Phase (same timeline)
- Actions:
  - ≥ 70%: bi-directional mapping between frameworks (e.g., NIST, ISO, IEC, NIS2, UK CAF).
...
- If a `[LOW_PARITY_OUTLIER]` is nevertheless highly relevant to Dynamic Risk → raise 🚩`[PROMPT_EVOLUTION_TRIGGER]` with suggested rule updates.

### 4.5 Output Instructions

- **Critical citation rule**:
  - Must include section header, paragraph number, or control identifier (e.g., “AC-3(10)”) *plus* page number to handle PDF vs printed-page offsets.
...
## 5. PROMPT_CHANGELOG – Evolution and Rigor
...
- **2026-02-21 – Source Rigor and Conflict Categorization Update**
  ...
  - Introduced `[INHERENT_FRICTION]`, `[SYSTEMIC_OMISSION]`, `[OUT_OF_SCOPE_SILENCE]` distinctions.
...
- Purpose:
  ...
  - Serve as Scout’s memory to avoid re-evaluating the same paper.
...
2. **Clarify role of the Dynamic Switch**  
   - Treat Tier 1–3 as an analytic lens, not assumed ground truth; allow documents to map to multiple tiers or none.

3. **Soften and contextualize some mappings and labels**  
   - Handle terms like “Privilege Escalation → Break-Glass” only in clear, contextually justified cases.  
   - Keep strong labels like `[SYSTEMIC_OMISSION]` as coding categories and reserve normative judgments for human analysis.

4. **Expand what “counts” for grey literature and discovery**  
   - Allow sources that discuss adaptive or conditional safety/security trade-offs, runtime risk decisions, or emergency operating modes even if they don’t use the exact phrase “Dynamic Risk”.

5. **Support “none/unknown” in the template**  
   - Let the auditor explicitly say “No explicit break-glass mechanism” or “Flexibility: Unknown/Not enough evidence” instead of inferring mechanisms where they aren’t clearly present.
...
7. **Enrich Scout’s memory log (optionally)**  
