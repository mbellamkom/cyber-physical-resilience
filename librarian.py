...
def run_audit():
    ...
    # V-11: Explicit checks for missing files
    if not rules_path.exists():
        print(f"[!] CRITICAL: Agent rules missing at {rules_path}")
        print("    Ensure the .agent directory exists and AGENT_RULES_PATH is correct.")
        return
    if not skills_path.exists():
        print(f"[!] CRITICAL: Agent skills missing at {skills_path}")
        return
